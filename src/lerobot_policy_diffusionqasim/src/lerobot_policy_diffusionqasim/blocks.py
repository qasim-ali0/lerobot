...
class SinusoidalTimeEmbedding(nn.Module):
    """Maps diffusion timesteps (B, 1) → sinusoidal embeddings (B, embed_dim)."""
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        # t: (B, 1)
        x = t.float() * self.freqs  # (B, half_dim) via broadcast
        return torch.cat([x.sin(), x.cos()], dim=-1)  # (B, embed_dim)
