...
class TransformerLayer2D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.self_attn = Attention2D(d_model, n_heads, dropout)
        self.norm1 = nn.LayerNorm(d_model)
...
class RoPE2D(nn.Module):
    def __init__(self, d, base=10000.0):
        super().__init__()
        assert d % 4 == 0          # need to split into 2 axes, each with sin/cos pairs
        self.d = d
        half = d // 2
        idx = torch.arange(0, half // 2).float()
        omegas = torch.exp(-2 * idx / half * math.log(base))
        self.register_buffer("omegas", omegas)   # (d//4,)

    def _rotate(self, x_half, pos):
        # x_half: (..., d//2), pos: (..., T)
        ang = pos[..., None] * self.omegas       # (..., T, d//4)
        cos, sin = ang.cos(), ang.sin()
        odds, evens = x_half[..., ::2], x_half[..., 1::2]
        return torch.stack([odds*cos - evens*sin, evens*cos + odds*sin], -1).flatten(-2)

    def forward(self, x, rows, cols):
        a, b = x[..., : self.d // 2], x[..., self.d // 2 :]
        return torch.cat([self._rotate(a, rows), self._rotate(b, cols)], dim=-1)
